...
export const GET: APIRoute = ({ url, request }) => {
  ...
  const body = isProduction
    ? [
        ...
        'Allow: /',
        '',
        '# /quick-start is the guide-delivery page: unlisted, out of the sitemap,',
        '# and carrying a noindex meta tag.',
        '#',
        '# DELIBERATELY *NOT* DISALLOWED (changed 2026-08-26). It was, and that was',
        '# counterproductive: Disallow blocks crawling, so Googlebot never fetches',
        '# the page and never sees the noindex it is supposed to obey. A blocked URL',
        '# can then still surface as a bare link with no snippet if anyone links to',
        '# it, and there is no way to get it removed because the directive telling',
        '# Google to drop it is unreachable.',
        '#',
        '# Letting crawlers fetch it means they read "noindex" and drop it for good.',
        '# Google says this explicitly: do not block a page you want deindexed.',
        '# Do not re-add a Disallow line here thinking it is safer. It is not.',
        '',
