...
1. In Vercel, add `alwaysbequitting.com` and `www.alwaysbequitting.com` to the project. **Use the exact A and CNAME values Vercel shows you** — they publish current values in the dashboard and they have changed over time, so never paste an IP from memory or from an old guide.
   ...
   🚩 **Set `www` as the PRIMARY domain, with the apex redirecting to it.** Vercel asks which way round. The site's canonical tags (`Base.astro`) and the sitemap declaration in `robots.txt.ts` both point at `https://www.alwaysbequitting.com`. Choosing the apex as primary would serve every page from a host its own canonical contradicts — precisely the wrong signal to send while trying to get indexed.

   **The two records to change at GoDaddy, and nothing else:**

   | Record | Currently | Change to |
   |---|---|---|
   | `A` @ | `3.33.251.168` **and** `15.197.225.128` | Vercel's A value — delete BOTH existing |
   | `CNAME` `www` | `dgtb6mhv7ir6.cloudfront.net` | Vercel's CNAME value |

   ⚠️ **Leave every other record alone.** `MX` @ (Google Workspace), `TXT` @ (SPF *and* Search Console verification), `TXT _dmarc`, `TXT resend._domainkey`, `TXT send`, `MX send`. All of those are email — breaking one costs mail, newsletters, or the contact form.
...
### Bing Webmaster Tools — not done, and it is not about Bing (added 2026-08-26)

**Why this exists.** ChatGPT's search leans on Bing's index. A site absent from
Bing is not merely missing Bing's own modest traffic, it is ineligible to be
cited in the assistant a large and growing share of people now ask first.
Search Console covers Google and nothing else. This is the other half, it is
free, and it is a one-time job.

**Good news first: `robots.txt` is already correct.** Production emits
`User-agent: *` / `Allow: /`, so `OAI-SearchBot` and `bingbot` are permitted.
Many sites block AI crawlers by accident and never find out. Nothing to change.

#### Route A — import the Google property. No code, no DNS, do this one.

Bing can import an already-verified Search Console property and marks it
verified automatically, which sidesteps verification entirely.

1. [bing.com/webmasters](https://www.bing.com/webmasters) → sign in.
2. **My Sites** → **Import**.
3. Sign in to Google and grant access when prompted.
4. Pick `alwaysbequitting.com` from the list → **Import**.

Microsoft's announcement: *"On successful completion, the selected sites will
be added and automatically verified in Bing Webmaster Tools."* Allow up to 48
hours before any traffic data appears.

⚠️ **Unverified detail: Search Console has two property types, and ours is a
Domain property.** Microsoft's documentation says "verified sites and sitemaps"
are imported without stating whether Domain properties are included alongside
URL-prefix ones. It may simply work. If `alwaysbequitting.com` does not appear
in the import list, that is why, and Route B is the fallback. This was not
guessed at further because the answer is one click away once someone is
actually signed in.

#### Route B — fallback only, and it CANNOT work before the cutover

Both site-side methods verify by reading the live HTML at
`alwaysbequitting.com`, which still serves **systeme.io**. Neither can succeed
until DNS moves. Do not attempt them early and conclude something is broken.

The meta tag route is already wired into `Base.astro`, no code change needed:

1. Bing Webmaster Tools → **Add site manually** → choose the **HTML Meta Tag**
   option → copy the `content` value out of
   `<meta name="msvalidate.01" content="…" />`.
2. Vercel → Project → Settings → Environment Variables → add
   `PUBLIC_BING_SITE_VERIFICATION` = that value. **One value per field** (the
   multi-line paste trap that cost an hour on `SYSTEME_API_KEY` applies here).
3. Redeploy, then click **Verify** in Bing.

Bing reads raw HTML via View Source, not JavaScript-rendered output. Astro
renders this tag server-side, so that is fine.

The other documented method is a `BingSiteAuth.xml` file at the domain root.
Not used here: the env-gated meta tag is the same shape as the existing Google
fallback and leaves no stray file in `public/`.

📄 **Not documented here on purpose:** Bing also offers a DNS verification
option. Bing's own `add-and-verify-site` help page could not be loaded
(2026-08-26, redirect loop), so the exact record type and value format are
unconfirmed, and **writing down a guessed DNS record for a domain under
`p=reject` DMARC is exactly the kind of speculation that breaks mail.** If
Routes A and B both fail, read the record format off Bing's own screen.

#### Sitemap — same sequencing rule as Google

⏳ **Submit `https://www.alwaysbequitting.com/sitemap-index.xml` AFTER the
Vercel cutover**, in Bing Webmaster Tools → **Sitemaps**. The systeme.io site
currently on the domain does not generate that file, so submitting early just
records a 404.

#### IndexNow — deliberately skipped for now

Bing Webmaster Tools includes IndexNow, which pings engines the moment a page
changes. It is genuinely useful for sites that publish constantly. This site
has seven pages that change rarely, so it would save days of discovery latency
a handful of times a year. Revisit only if a content section is ever built.
